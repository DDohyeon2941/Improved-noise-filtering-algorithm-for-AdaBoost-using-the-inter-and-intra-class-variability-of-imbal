...
class Proposed_Filter(Base_Filter):
    ...
    def _detect_noise(self, sample_weight, y):
        ...
        if self.noise_rule == 'case1':
            ...
            c1_thr, c1_noise = self._detect_noise_1(sample_weight, y)
            c2_thr, c2_noise = self._detect_noise_2(sample_weight, y)

            if self.noise_rule == 'and':
                final_noise = np.intersect1d(c1_noise, c2_noise)
                final_thr = c1_thr + c2_thr
            elif self.noise_rule == 'or':
                final_noise = np.union1d(c1_noise, c2_noise)
                final_thr = c1_thr + c2_thr
