...
if __name__ == '__main__':
    ...
    result_df = pd.DataFrame(columns=['dataset','rel_rs','abs_rs','fold','prob','noise_level','acc', 'f1', 'pre', 'recall', 'auc_score', 'g_mean', 'spec'])
    i=0
        ...
        for rel_rs, abs_rs in enumerate(random_states):
            ...
            for f_idx in range(fold_num):
                for uni_prob, uni_ratio in itertools.product(prob_condi, noise_levels):
                    ...
                    result_df.loc[i] = [fname.split('.')[0], rel_rs, abs_rs, f_idx, uni_prob, uni_ratio] + scores
                    i+=1
                    clf =None
    ...
    #result_df.to_csv(r'prediction_performance_adaboost_noise_injected_dataset.csv', index=False)
    
    #result_df.groupby(['dataset','prob','noise_level']).mean()['auc_score'].unstack()
